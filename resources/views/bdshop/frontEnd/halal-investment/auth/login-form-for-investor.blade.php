          ...
          <div class="row">
            <div class="col-md-4 col-sm-12">
              <div class="form-card card shadow border-0 p-3">
                ...
                <form action="{{route('login.form.for.investor.submit')}}" method="POST">
                  @csrf
                    <div class="mb-3">
                        <label for="" class="fw-bolder mb-2">Email</label>
                        <input type="text" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email')}}">
                        <input type="text" name="company_id" value="{{ $company_id }}" hidden>
                        <input type="text" name="investor_amount" value="{{ $investor_amount }}" hidden>
                        <input type="text" name="company_name" value="{{ $company_name->company_name }}" hidden>
