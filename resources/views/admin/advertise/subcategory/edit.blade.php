                           ...
                           <form action="{{route('admin.sub-categories.update',$subcategory->id )}}" method="POST" enctype="multipart/form-data">
                               ...
                               <div class="row">
                                   <div class="col-md-6 mt-2">
                                       ...
                                       <input type="text" class="form-control" value="{{ $subcategory->name }}" name="name" placeholder="Name" required>
                                   </div>
                                   <div class="col-md-6 mt-2">
                                        ...
                                        <select name="category_id" id="category_id" class="form-control">
                                            ...
                                            @foreach ($categories as $item)
                                                <option value="{{$item->id}}" <?php if($subcategory->category_id == $item->id) echo "selected";?>>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                   <div class="col-md-6 mt-2">
                                       <label>Icon </label>
                                       <img src="{{ asset($subcategory->icon) }}" alt="" width="40px">
                                       <input type="file" class="form-control" name="icon">
                                   </div>
                                   <div class="col-md-6 mt-2">
                                    <label>Meta Title</label>
                                    <input type="text" class="form-control" value="{{ $subcategory->meta_title }}" name="meta_title" placeholder="Meta Title">
                                    </div>
