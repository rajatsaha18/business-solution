               ...
               <div class="col-10 offset-md-1">
                   <div class="card">
                       <div class="card-header">
                           ...
                           <a href="{{route('admin.sub-sub-categories.index')}}" class="btn btn-primary float-right" >
                                Back
                           </a>
                       </div>
                       <div class="card-body">
                           <form action="{{route('admin.sub-sub-categories.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                               @csrf
                               @method('patch')
                                ...
                                <div class="col-md-12 mt-2">
                                    ...
                                    <select name="subcategory_id" id="subcategory_id" class="form-control" required>
                                        ...
                                        @foreach ($subcategories as $item)
                                            <option value="{{$item->id}}" {{$item->id == $data->subcategory_id ?'selected':''}}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-12 mt-2">
